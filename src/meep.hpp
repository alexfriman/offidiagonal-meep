...
typedef struct {
  ...
  std::complex<double> exp_decay;
  std::complex<double> exp_decay_zz;
  realnum Pr_diag[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS];
  realnum Pr_offdiag[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS];
  realnum Pr_z[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS];

  
  realnum *P[NUM_FIELD_COMPONENTS][2];
  realnum *P_prev[NUM_FIELD_COMPONENTS][2];
  realnum *D_prev[NUM_FIELD_COMPONENTS][2];
  std::complex<double>  *conv_offdiag1[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS][2];
  std::complex<double>  *conv_diag1[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS][2];
  
  std::complex<double> *conv_offdiag2[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS][2];
  std::complex<double> *conv_diag2[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS][2];
  
  std::complex<double> *conv_z1[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS][2];
  std::complex<double> *conv_z2[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS][2];
  
  std::complex<double> *chi0_diag1;
  std::complex<double> *chi0_offdiag1;
  std::complex<double> *chi0_offdiag2;//1/(1+ (chi_xy)^2/((1+chi_xx)(1+chi_yy)))
  std::complex<double> *chi0_diag2;//1/((1+chi_xx)(1+chi_yy))
  std::complex<double> *chi0_zz1;//1/(1+ (chi_xy)^2/((1+chi_xx)(1+chi_yy)))
  std::complex<double> *chi0_zz2;//1/((1+chi_xx)(1+chi_yy))
  realnum data[1];
} gyroelectric_susceptibility_data;

class gyroelectric_susceptibility : public susceptibility {
public:
    gyroelectric_susceptibility(double in_nu_col, double in_nu_cycl, double b_dirx, double b_diry, double b_dirz);
    virtual susceptibility *clone() const { return new gyroelectric_susceptibility(*this); }
    virtual ~gyroelectric_susceptibility() {}
    ...
    //virtual void rowM_to_E_field(realnum* rM, realnum twoD[NUM_FIELD_COMPONENTS][NUM_FIELD_COMPONENTS]) const;
    ...
    virtual void init_internal_data(realnum *W[NUM_FIELD_COMPONENTS][2],
			  double dt, const grid_volume &gv, void *data) const;
    //virtual void init_projections(realnum *W[NUM_FIELD_COMPONENTS][2]);
    virtual void *copy_internal_data(void *data) const;
...
protected:
    ...
    //realnum b_dirz;
    realnum R[3][3];
    //these are projection matrices in Cartesian coordinates. They are used only to establish field coordinates matrices. x -> Ex or Hx. Sorry for my poor English, hope you understad the idea
    realnum Pr_diag_cart[3][3];
    realnum Pr_offdiag_cart[3][3];
    realnum Pr_z_cart[3][3];
